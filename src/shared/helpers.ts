...
export class GreetingHelper {
    ...
    public static randomNominationMessage(
        ...
        const replies = [
            ...
            `🦀 I'm running out of things to type here, quick ${user} distract them! 🦀`,
            ...
            `🦀 This thing is still working ${user}? Don't tell Chris, he might remember it exists and try adding some new "features" to it. 🦀`,
            `🦀 Hot off the archives: ${user}'s featured community moment! 🦀`,
            `🦀 Behold! Today's gem from the Nordev collection has arrived! 🦀`,
            `🦀 Is that the sonic boom from our USAF mates? Oh no it's just another banger from ${user}! 🦀`,
            `🦀 The algorithm has spoken! ${user} has calimed victory for today's featured message! 🦀`,
            `🦀 My daily treasure hunt through the archives yielded this gem from our beloved ${user}! 🦀`,
